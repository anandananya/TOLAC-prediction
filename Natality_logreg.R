...
#Write CSV
fwrite(final_data, "/Users/anishamittal/Desktop/Carle/Year 4/Data Science/natality_2022_final.csv")
...
final_data <- read.csv("/Users/anishamittal/Desktop/Carle/Year 4/Data Science/natality_2022_final.csv", stringsAsFactors = TRUE)
final_data$Birth.Place <- factor(final_data$Birth.Place, levels = c("Hospital", "Freestanding birth center", "Home (intended)", "Home (not)", "Home (unknown)", "Clinic", "Other"))
final_data$Mother.s.Race.Hispanic <- factor(final_data$Mother.s.Race.Hispanic, levels = c("White", "Black", "AIAN", "Asian", "NHOPI", "multirace", "Hispanic"))
final_data$Mother.s.education <- factor(final_data$Mother.s.education, levels = c("8th grade or less", "9-12th grade", "High school/GED", "College credit", "Associate degree", "Bachelor's", "Master's", "Doctorate"))
final_data$TOLAC.Attempted..if.cesarean. <- factor(final_data$TOLAC.Attempted..if.cesarean., levels = c("Y", "Not applicable"))
final_data$Delivery.Method <- factor(final_data$Delivery.Method, levels = c("VBAC", "Repeat C-section"))
final_data$Payment <- factor(final_data$Payment, levels = c("Medicaid", "Private Insurance", "Self-Pay", "Indian Health Service", "CHAMPUS/TRICARE", "Other Gov", "Other"))
...
logistic_model <- glm(`Delivery_Method_Binary` ~ `Birth Place` + `Mother's Age` + `Mother's Race/Hispanic` + `Mother's education` + `Prior births now living`
                      + `Prior births now dead` + `Interval Since Last Live Birth` + `Number of Prenatal Visits` + `Cigarettes before pregnancy`
                      + `1st Tri Cigarettes` + `2nd Tri Cigarettes` + `3rd Tri Cigarettes` + `Pre-pregnancy BMI` + `Weight gain` + `Pre-pregnancy diabetes`
                      + `Gestational Diabetes` + `Pre-pregnancy HTN` + `Gestational HTN` + `Previous Preterm Birth` + `Number of Previous Cesareans` + `Gonorrhea` + `Syphilis`
                      + `Chlamydia` + `Hep B` + `Hep C` + `Payment` + `Obstetric Estimate`, 
                      data = train_data, 
                      family = binomial)
...
cv_model <- train(Delivery_Method_Binary ~ Birth.Place + Mother.s.Age + Mother.s.Race.Hispanic + Mother.s.education + Prior.births.now.living
                  + Prior.births.now.dead + Interval.Since.Last.Live.Birth + Number.of.Prenatal.Visits + Cigarettes.before.pregnancy
                  + X1st.Tri.Cigarettes + X2nd.Tri.Cigarettes + X3rd.Tri.Cigarettes + Pre.pregnancy.BMI + Weight.gain + Pre.pregnancy.diabetes
                  + Gestational.Diabetes + Pre.pregnancy.HTN + Gestational.HTN + Previous.Preterm.Birth + Number.of.Previous.Cesareans + `Gonorrhea` + `Syphilis`
                  + `Chlamydia` + Hep.B + Hep.C + `Payment` + `Obstetric.Estimate`, 
                  data = final_data2, 
                  method = "glm", 
